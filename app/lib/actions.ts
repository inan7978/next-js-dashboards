...
const FormSchema = z.object({
  ...
  amount: z.coerce
    .number()
    .gt(0, { message: "Please enter an amount greater than $0." }), // coerce "changes" the value to a number while also validating the type
...
export async function createInvoice(prevState: State, formData: FormData) {
  ...
  revalidatePath("/dashboard/invoices"); // since you have new data now, ^^^^, you need to clear the cache for the invoices route and trigger a new request to the server.
  // that is when revalidatePath() comes in handy
  redirect("/dashboard/invoices"); // this simply redirects the user back to this page
}
